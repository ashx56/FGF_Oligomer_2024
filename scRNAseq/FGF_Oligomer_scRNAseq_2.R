...
# Processed data files can be downloaded from Harvard Dataverse doi:10.7910/DVN/HMCSIB
# Load raw counts, cell metadata
counts <- readMM("FGF_Oligomer_scRNAseq_2_filtered_counts.mtx")
genes <- read.csv("FGF_Oligomer_scRNAseq_2_genes.tsv", header = F)
barcodes <- read.csv("FGF_Oligomer_scRNAseq_2_barcodes.tsv", header = F)

rownames(counts) <- genes[, 1]
colnames(counts) <- barcodes[, 1]

mdata <- read.csv("FGF_Oligomer_scRNAseq_2_metadata.csv", row.names = 1)
...
plot_cells(cds_FGF, color_cells_by = "cluster")
...
# Endothelial/Perivascular proportions grouped by D2-5 treatment
# Extract metadata
...
# Endogenously secreted FGFs (Fig S23C)
...
# Read raw counts; data available at GEO: BioProject PRJNA837932 
av_counts <- read.delim("PRJNA837932_counts.txt", 
...
# Classify endothelial subtypes based on A-V specificity
...
# Visualize each sample as density plot
